...
$sql = "SELECT * FROM exchange_results WHERE id=$id";

$query = mysqli_query($connect, $sql);

while ($row = mysqli_fetch_assoc($query)) :
?>
    <form action="./exchangeUpdate.php" method="post">
        <input type="hidden" name="id" value="<?= $row["id"] ?>">
        <div class=" mt-5">
            <div class="mb-6">
                <label for="default-input" class="block mb-2 text-lg font-bold text-gray-900 dark:text-white">Amount</label>
                <input value="<?= $row["amount"] ?>" required type="number" id="default-input" name="amount" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            </div>
